...
from telethon.errors import FloodWaitError, UserDeactivatedError, AuthKeyDuplicatedError,     SessionPasswordNeededError, PhoneCodeInvalidError, PhoneCodeExpiredError
...
class AccountManager:
    def __init__(self):
        ...
        self.flood_waits: Dict[int, int] = {}  # account_id -> until_timestamp
    ...
    async def _check_accounts(self):
        """Проверка статуса всех активных аккаунтов"""
        ...
        for acc in accounts:
            ...
            # Если клиент подключен — проверяем соединение
    ...
    async def add_account(self, phone: str, code: str = None, password: str = None) -> Dict:
        """Добавление нового аккаунта (только owner)"""
    ...
    async def get_account_dialogs(self, account_id: int) -> List[Dict]:
        """Получение списка чатов и папок аккаунта"""
    ...
    async def forward_post(self, account_id: int, from_chat: int, message_id: int, to_chat: int) -> Dict:
        """Пересылка поста методом forward (сохраняет медиа, эмодзи, форматирование)"""
        if account_id not in self.clients:
    ...
    async def send_broadcast(self, post_id: int, account_id: int, chat_ids: List[int], 
        ...
        """Рассылка поста с очередью и задержкой"""
        if account_id not in self.clients:
            return
        ...
        for chat_id in chat_ids:
            ...
            delay = asyncio.random.randint(interval[0], interval[1]) if len(interval) == 2 else interval[0]
