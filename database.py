...
class Database:
    ...
    def update_account_api(self, account_id: int, api_id: str, api_hash: str):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE accounts SET api_id = ?, api_hash = ? WHERE id = ?", (api_id, api_hash, account_id))

    def add_account(self, phone: str, session_name: str, api_id: str = None, api_hash: str = None) -> bool:
        try:
            with self._connect() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO accounts (phone, session_name, api_id, api_hash) VALUES (?, ?, ?, ?)",
    ...
    def update_account_status(self, account_id: int, status: str, flood_wait: int = 0):
        with self._connect() as conn:
            ...
            cursor.execute("UPDATE accounts SET status = ?, flood_wait_until = ? WHERE id = ?", (status, flood_until, account_id))

    def increment_account_load(self, account_id: int):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE accounts SET load_count = load_count + 1, last_used = ? WHERE id = ?", (int(datetime.now().timestamp()), account_id))
    ...
    def set_premium(self, user_id: int, until: datetime):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE users SET is_premium = 1, premium_until = ? WHERE user_id = ?", (until.isoformat(), user_id))
    ...
    def create_pending_post(self, user_id: int, message_id: int) -> int:
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO pending_posts (user_id, message_id) VALUES (?, ?)", (user_id, message_id))
    ...
    def get_user_posts(self, user_id: int) -> List[Dict]:
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM pending_posts WHERE user_id = ? ORDER BY created_at DESC", (user_id,))
            return [dict(row) for row in cursor.fetchall()]

    def add_folder(self, user_id: int, name: str, chat_ids: str):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO chat_folders (user_id, name, chat_ids) VALUES (?, ?, ?)", (user_id, name, chat_ids))
            return cursor.lastrowid
    ...
    def add_payment(self, user_id: int, payment_id: str, amount: float):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO payments (user_id, payment_id, amount) VALUES (?, ?, ?)", (user_id, payment_id, amount))

    def update_payment(self, payment_id: str, status: str):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE payments SET status = ?, paid_at = ? WHERE payment_id = ?", (status, datetime.now().isoformat() if status == "succeeded" else None, payment_id))
    ...
    def add_log(self, post_id: int, account_id: int, chat_id: int, status: str, error_msg: str = None):
        with self._connect() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO send_logs (post_id, account_id, chat_id, status, error_msg) VALUES (?, ?, ?, ?, ?)", (post_id, account_id, chat_id, status, error_msg))
