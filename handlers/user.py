...
async def cmd_start(message: Message):
    ...
    welcome_text = (
        f"{STAR} <b>Добро пожаловать в авторассылку!</b>\n\n"
        f"{SEND} Здесь вы можете разослать свои посты по чатам.\n"
        f"{INFO} Отправьте пост боту — он пройдет модерацию и будет разослан.\n\n"
    )
    ...
    await message.answer(
        welcome_text,
        reply_markup=main_menu_kb(is_premium, is_owner),
        parse_mode="HTML"
    )
...
async def cb_send_post(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        f"{SEND} <b>Отправьте пост для рассылки</b>\n\n"
        f"{INFO} Бот перешлет его в канал, откуда аккаунты разошлют по чатам.\n"
        f"{WARNING} Пост пройдет модерацию перед отправкой.",
...
async def process_post(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"{CROSS} Ошибка пересылки в канал: {{e}}",
    ...
    # Отправляем на модерацию owner'ам
    mod_text = (
        f"{BELL} <b>Новый пост на модерацию!</b>\n\n"
        f"{EYES} От: <a href=\"tg://user?id={{message.from_user.id}}\">{{message.from_user.first_name}}</a>\n"
        f"{INFO} ID поста: <code>{{post_id}}</code>\n\n"
        f"{WARNING} Одобрить рассылку?"
    )
    ...
    await message.answer(
        f"{CHECK} Пост отправлен на модерацию!\n"
        f"{INFO} Вы получите уведомление после проверки.",
        reply_markup=main_menu_kb(db.is_premium_active(message.from_user.id)),
        parse_mode="HTML"
    )
    await state.clear()
...
async def cb_my_posts(callback: CallbackQuery):
    ...
    if not posts:
        await callback.message.edit_text(
            f"{INFO} У вас пока нет рассылок.",
            reply_markup=back_kb()
        )
        return
    ...
    for post in posts[:10]:
        status_emoji = {
            ...
            "completed": CHECK,
            "stopped": PAUSE
        }.get(post["status"], QUESTION)

        text += f"{{status_emoji}} Пост #{{post['id']}} — <b>{{post['status']}}</b>\n"

    # Клавиатура с последним постом для управления
    last_post = posts[0]
    await callback.message.edit_text(
        text,
        reply_markup=post_control_kb(last_post["id"], last_post["status"]),
        parse_mode="HTML"
    )
...
async def cb_post_status(callback: CallbackQuery):
    ...
    text = (
        f"{CHART} <b>Статус рассылки #{{post_id}}</b>\n\n"
        f"{EYES} Статус: <b>{{post['status']}}</b>\n"
        f"{GEAR} Аккаунты: {{post['selected_accounts']}}\n"
        f"{FOLDER} Папки: {{post['selected_folders']}}\n"
        f"{SPEECH} Чаты: {{post['selected_chats']}}"
    )
...
# === ВЫБОР АККАУНТОВ И ЧАТОВ ===
async def start_broadcast_setup(callback: CallbackQuery, post_id: int):
    """Настройка рассылки после одобрения"""
    accounts = db.get_accounts(status="active")
    if not accounts:
        await callback.message.edit_text(
            f"{CROSS} Нет доступных аккаунтов для рассылки.",
            reply_markup=back_kb()
        )
        return

    await callback.message.edit_text(
        f"{EYES} <b>Выберите аккаунты для рассылки:</b>",
        reply_markup=select_accounts_kb(accounts),
        parse_mode="HTML"
    )
...
async def cb_folders(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"{FOLDER} <b>Ваши папки</b>\n\n"
        f"{INFO} Управляйте группами чатов для рассылки.",
...
async def cb_folder_create(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        f"{PLUS} <b>Создание папки</b>\n\n"
        f"{INFO} Введите название папки:",
        reply_markup=back_kb("menu_folders"),
        parse_mode="HTML"
    )
...
async def process_folder_name(message: Message, state: FSMContext):
    ...
    # Создаем папку с пустым списком чатов
    db.add_folder(message.from_user.id, name, "[]")
    await message.answer(
        f"{CHECK} Папка <b>\"{{name}}\"</b> создана!\n"
        f"{INFO} Теперь добавьте чаты через панель.",
        reply_markup=main_menu_kb(db.is_premium_active(message.from_user.id)),
        parse_mode="HTML"
    )
    await state.clear()
...
async def cb_folder_view(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text,
        reply_markup=folder_detail_kb(folder_id),
        parse_mode="HTML"
    )
...
async def cb_help(callback: CallbackQuery):
    text = (
        f"{INFO} <b>Помощь</b>\n\n"
        f"{SEND} <b>Как разослать пост?</b>\n"
        f"1. Нажмите {SEND} Разослать пост\n"
        f"2. Отправьте пост боту\n"
        f"3. Дождитесь модерации\n"
        f"4. Выберите аккаунты и чаты\n"
        f"5. Запустите рассылку\n\n"
        f"{DIAMOND} <b>Премиум</b> дает:\n"
        f"• Выделенный аккаунт\n"
        f"• Нет очереди\n"
        f"• Настройка интервалов (до 20 сек)\n"
        f"• Собственные папки чатов\n\n"
        f"{WARNING} Все рассылки проходят модерацию."
    )
    await callback.message.edit_text(
        text,
        reply_markup=back_kb(),
...
async def start_broadcast(post_id: int, bot):
    ...
    # Распределяем чаты между аккаунтами
    ...
    for i, acc_id in enumerate(accounts_ids):
        ...
        asyncio.create_task(
            account_manager.send_broadcast(
                post_id, acc_id, acc_chats, channel_id, channel_msg_id
            )
        )
